...
// Query: *[_type == 'startup' && defined(slug.current) && !defined($search) || category match $search || title match $search || author->name match $search] | order(_createdAt desc) {      _id,      _createdAt,      title,      slug,      author -> { _id, name },      views,      description,      category,      image,    }
...
// Query: *[_type == 'startup' && _id == $id][0] {    _id,    title,    slug,    _createdAt,    author -> {      _id, name, username, image, bio , email    },    views,    description,    category,    image,    pitch  }
...
// Query: *[_type == 'author' && id == $id][0]{      _id,      id,      name,      username ,       image ,       bio,      email,    }
...
// Variable: AUTHOR_BY_OWN_ID_QUERY
// Query: *[_type == 'author' && _id == $id][0]{      _id,      id,      name,      username ,       image ,       bio,      email,    }
export type AUTHOR_BY_OWN_ID_QUERYResult = {
  _id: string;
  id: number | null;
  name: string | null;
  username: string | null;
...
// Query: *[_type == 'startup' && author._ref == $id] | order(_createdAt desc) {      _id,      _createdAt,      title,      slug,      author -> { _id, name },      views,      description,      category,      image,    }
